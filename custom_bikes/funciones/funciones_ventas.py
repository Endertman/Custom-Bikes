    ...
    try:
        with open('datos/pedido_respaldo.csv', 'r') as pedidos_file:
            ...
            for pedidos_row in pedidos_reader:
                ...
                with open ('datos/tecnico_pedido_respaldo.csv') as tecnico_pedido_file:
                    ...
                    for tecnico_pedido_row in tecnico_pedido_reader:
                        tecnico_id = tecnico_pedido_row[0]
                        pedido_id = tecnico_pedido_row[1]

                        try:
                            cursor.execute('''INSERT INTO tecnico_pedido (tecnico_id, pedido_id) VALUES (?, ?)''', (tecnico_id, pedido_id))
                            conn.commit()
                ...
                with open ('datos/cotizacion_codigo_respaldo.csv') as cotizacion_codigo_file:
                ...
                with open ('datos/historico_pedido_respaldo.csv') as historico_pedido_file:
                    ...
                    for historico_pedido_row in historico_pedido_reader:
                        ...
                        try:
                            cursor.execute('''INSERT INTO historico_pedido (id_pedido, fecha_entrega_pedido) VALUES (?, ?, ?)''', (pedido_id, fecha_entrega))
                            conn.commit()
                            print(f"Estado del pedido {id_pedido} actualizado correctamente.")

                        except sqlite3.Error as e:
                            print(f"Error al actualizar el estado del pedido {id_pedido}: {e}")
                
                with open ('datos/garantia_respaldo.csv') as garantia_file:
                    ...
                    for garantia_row in garantia_reader:
                        ...
                        try:
                            cursor.execute('''INSERT INTO garantia (id_pedido, fecha_inicio, fecha_fin, cobertura, condiciones, estado) VALUES (?, ?, ?, ?, ?, ?)''', (pedido_id, fecha_inicio, fecha_fin, cobertura, condiciones, estado))
                            conn.commit()
                            print(f"Garantía para el pedido {id_pedido} agregada correctamente.")

                        except sqlite3.Error as e:
                            print(f"Error al agregar la garantía para el pedido {id_pedido}: {e}")
